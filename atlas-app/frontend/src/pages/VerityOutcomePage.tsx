/*
 * VerityOutcomePage — Post-build outcome page for the dbt-wizard demo.
 *
 * Route: /outcome
 *
 ...
 *
 * Ported from Healthcare-EPIC-Snowflake-Demo/ClarityOutcomePage.tsx — Insurance-flavored.
 */
...
export default function VerityOutcomePage() {
  const [o, setO] = useState<OutcomeData | null>(null);

  useEffect(() => {
    fetch(import.meta.env.BASE_URL?.replace(/\/$/, '') + '/data/outcome.json')
      .then(r => {
        if (!r.ok) throw new Error(`Failed to load outcome.json: ${r.status}`);
        return r.json();
      })
      .then(setO)
      .catch(() => {});
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          ...
          >
            <span className="h-2 w-2 rounded-full bg-[var(--clinical-green,#145e36)] animate-pulse" />
            Build · Materialized
          </span>
        ...
        <p className="mt-3 max-w-3xl leading-relaxed text-lg text-[var(--ink-muted)]">
          ...
          {o.build_seconds} seconds — the same window the Reserves Committee waited for an answer.
        </p>
      </header>
      ...
      <section
        ...
        style={{ borderLeft: '5px solid var(--gold)', background: 'rgba(184,151,92,0.04)' }}
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <div className="eyebrow mb-1">Over-model claims</div>
            <div className="font-serif text-3xl font-semibold" style={{ color: 'var(--gold-dim)' }}>
          ...
          <div>
            <div className="eyebrow mb-1">Clause signature</div>
            <div className="font-mono text-sm font-semibold text-[var(--ink-strong)]">
              tier-2 commercial wind · debris_removal_sublimit = $25K · unchanged since 2024
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-10">
        <LineagePanel
          ...
          subtitle="No gold table tracks CAT wind severity by tier, region, and day."
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <div className="rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm sm:col-span-2"
          ...
          <div className="font-mono text-xs text-[var(--ink-muted)]">
            {o.row_count.toLocaleString()} rows · Snowflake native table · auto-suspend compute
          </div>
        </div>
...
function WizardColumn({ data, tone }: { data: Column; tone: 'crisis' | 'resolved' }) {
  const toneColor =
    tone === 'crisis' ? 'var(--gold-dim, #b8975c)' : '#145e36';
  const chipClass =
